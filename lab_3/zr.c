...
void get_buffer(zb_uint8_t patam) ZB_CALLBACK;
...
void bulb_sent_on_command(zb_uint8_t param) ZB_CALLBACK
{
    ...
    zb_uint8_t *ptr = (zb_uint8_t*)buf,
               command = ON,
               addr = *((zb_uint8_t*)ZB_GET_BUF_TAIL(buf, sizeof(zb_uint8_t)));
    ...
    if(ZB_BUF_LEN(buf) < sizeof command)
    {
        ZB_BUF_ALLOC_LEFT(buf, (sizeof command) - ZB_BUF_LEN(buf), ptr);
    }
    ptr[0] = (zb_uint8_t)command;
...
void bulb_sent_color_command(zb_uint8_t param) ZB_CALLBACK
{
    ...
    zb_uint8_t *ptr = (zb_uint8_t*)buf,
               command = COLOR;
    struct addr_and_color tail = *((struct addr_and_color*)ZB_GET_BUF_TAIL(buf, sizeof(struct addr_and_color)));
    zb_apsde_data_req_t *req = ZB_GET_BUF_TAIL(buf, sizeof(zb_apsde_data_req_t));
    ...
    if(ZB_BUF_LEN(buf) < (sizeof command) + (sizeof tail.color))
    {
        ZB_BUF_ALLOC_LEFT(buf, (sizeof command) + (sizeof tail.color) - ZB_BUF_LEN(buf), ptr);
    }
    memcpy(ptr, &command, sizeof command);
    memcpy(ptr + (sizeof command), &(tail.color), sizeof tail.color);
  
    TRACE_MSG(TRACE_APS3, "Sending apsde_data.request, command: %x, color: %x", (FMT__D_D, command, tail.color));
    ZB_SCHEDULE_CALLBACK(zb_apsde_data_request, ZB_REF_FROM_BUF(buf));
}
...
void send_data(zb_uint8_t param) ZB_CALLBACK
{
    zb_buf_t *buf = (zb_buf_t*)ZB_BUF_FROM_REF(param);
    zb_uint8_t *ptr;
    struct addr_and_color* tail;
    
    ZB_BUF_INITIAL_ALLOC(buf, 1, ptr);
    tail = ZB_GET_BUF_TAIL(buf, sizeof(struct addr_and_color));

    tail->addr = 0x0000;
    tail->color = 0xdeadbeef;    
